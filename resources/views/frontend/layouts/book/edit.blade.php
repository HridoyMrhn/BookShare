    ...
    <div class="row">
        <div class="card shadow mb-4 mt-4">
            <div class="card-header py-3">
                <h3 class="m-0 d-inline-block font-weight-bold text-primary">Edit {{ $book->book_name }}</h3>
            </div>
            <div class="card-body">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-lg-6">
                                ...
                                <select name="author_id[]" id="author_id" class="multiple-select form-control" multiple>
                                    ...
                                    @foreach ($authors as $data)
                                        ...
                                        {{-- <option value="{{ $data->id }}" {{ in_array($data->id, old("author_id") ?: []) }} selected="true">{{ $data->author_name }}</option> --}}
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group col-lg-6">
                                ...
                                <select name="translator_id" id="translator_id"  class="form-control">
                                    ...
                                    @foreach ($book_tb as $data)
                                        <option value="{{ $data->id }}" {{ $book->translator_id == $data->id ? 'selected':'' }}>{{ $data->book_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-lg-6">
